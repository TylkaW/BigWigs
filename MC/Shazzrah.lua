...
function BigWigsShazzrah:BigWigs_RecvSync(sync, rest, nick)
	if sync == self:GetEngageSync() and rest and rest == boss and not started then
		started = true
		self:ScheduleEvent("bwrshazzrahinitcs", self.InitCounterspell, 15, self)
		self:ScheduleEvent("BigWigs_Message", 12, "Contresort dans 3 secondes !", "Urgent")
		self:TriggerEvent("BigWigs_StartBar", self, "Contresort", 15, "Interface\\Icons\\Spell_Frost_Iceshock")
...
function BigWigsShazzrah:Counterspell()
	self:ScheduleEvent("BigWigs_Message", 14, "Contresort dans 3 secondes !", "Urgent")
	self:TriggerEvent("BigWigs_StartBar", self, "Contresort", 17, "Interface\\Icons\\Spell_Frost_Iceshock")
end

function BigWigsShazzrah:InitCounterspell()
	self:ScheduleRepeatingEvent("bwloathebSporeSpawn", self.Counterspell, 17, self)
	self:Counterspell()
end