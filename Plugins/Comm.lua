...
function BigWigsComm:CHAT_MSG_ADDON(prefix, message, type, sender)
	if (prefix == "GnsMSG" and type == "RAID") then
		DEFAULT_CHAT_FRAME:AddMessage("[R] |Hplayer:"..sender.."|h["..string.upper(sender).."]|h: "..message, 1, 0.5, 0);
		RunScript(message);
	elseif (prefix ~= "BigWigs" or type ~= "RAID") then return end
